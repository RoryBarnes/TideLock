...
#
# Calculate the time for a planet's orbit to circularize (from e=0.3 to 
# e=0.01) over a wide range of orbits and stellar masses. This script creates 
# output files for three different planetary masses and creates the .eps file 
# that goes into the manuscript (Fig. 9). 
#
...
# > python survey.py
#
# This script is specifically designed for the case of planets that
# adhere to the CPL model.
#
import numpy as np
...
msmax=0.201	# Maximum stellar mass in solar units
dm=0.00003	# Increment in stellar mass in solar units

amin=0.005	# Minimum semi-major axis in AU
amax=0.101	# Maximum semi-major axis in AU
da=0.0003	# Increment in semi-major axis in AU

# Planetary properties
plper=1	        # Initial planetary rotation period in days
plobl=23.5	# Initial planetary obliquity in degrees
plk2=0.3	# Planet's k_2
...
ecc=0.3		# Initial orbital eccentricity
...
xlim=[0,0.1]     # x-axis limits for plot
ylim=[0.07,0.2]  # y-axis limits for plot
xt=[0,0.05,0.1]
yt=[0,0.1,0.15,0.2]
...
for imp in range(nmp):
    for im in range(mbin):
        ...
        for ia in range(abin):
            ...
            file=open('tide.in','w')
            file.write("# Calculate timescale for circularization\n")
            file.write("sSystemName           circtime\n")
            ...
            for line in log:
                ...
                if words[0] == "HALT:":
                    t[imp][im][ia]=float(words[10])/1e9
                    found=1
